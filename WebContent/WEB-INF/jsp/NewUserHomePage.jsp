 ...
 <%} %>
      ...
      <main class="page-content">
        <div class="row">
			...
			<div class="container">
				<p>As a new Member here are all the clubs that are open right now</p>
				<br>
				<p>Click on each button to learn more about that club</p>
				<br>
				<p>After you have found one you would like to join. Check the one you would like and click submit</p>
				...
				<%while(index < size) { %>
					<button type="button" class="btn btn-info" data-toggle="collapse" data-target="#<%=index%>"><%=clubList.get(index).getClub_name() %></button>
					<div id="<%=index%>" class="collapse">
						<div class="well"><p><%=clubList.get(index).getClub_description() %></p></div>
				...
				<%LinkedList<Club> clubList2 = (LinkedList<Club>) request.getAttribute("clubList");
					int index2 = 0;
					int size2 = clubList2.size();
				
				%>
				
				<form action="NewUserHomePageServlet" method="POST">
					<%while(index2 < size2) { %>
						<input type="radio" name="club_id_num" value="<%=clubList2.get(index2).getClub_id_num()%>"><%=clubList2.get(index2).getClub_name()%><br>
					...
					<h3>You will need to log in again after this</h3>
